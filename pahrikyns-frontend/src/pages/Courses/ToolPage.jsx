...
export default function ToolPage() {
  ...
  // Helper to get localized string or fallback to string
  const getL = (v) => {
    if (!v) return "";
    if (typeof v === "string") return v;
    return v[langKey] || v["en"] || "";
  };
  ...
  return (
    <Box sx={{ position: "relative", minHeight: "100vh", bgcolor: "#0f172a", color: "white", overflow: "hidden" }}>
            ...
            <Grid item xs={12} md={7}>
              ...
              <Box display="flex" gap={3} mb={4}>
                ...
                <Box display="flex" alignItems="center" gap={1}>
                  ...
                  <Typography fontWeight={900} color="#fff">4.9 (Recent)</Typography>
                </Box>
              </Box>
      ...
      <Container maxWidth="lg" sx={{ pb: 10, position: "relative", zIndex: 1 }}>
              ...
              <Grid item xs={12} md={6} key={l.num}>
                <Paper sx={{
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="start" mb={2}>
                    <Box>
                      <Typography variant="subtitle2" color="#00eaff" mb={0.5}>
                        {langKey === 'ta' ? `பாடம் ${l.num}` : `LESSON ${l.num}`}
                      </Typography>
                      <Typography variant="h6" fontWeight={700} color="white">{getL(l.meta.title) || `Lesson ${l.num}`}</Typography>
                    </Box>
                  ...
                  <Typography variant="body2" color="rgba(255,255,255,0.6)" mb={3}>
                    {getL(l.meta.description) || "Dive into the details of this module."}
                  </Typography>
        ...
        {/* REVIEW SYSTEM */}
        {course?.id && <ReviewSystem courseId={course.id} />}
      </Container>
