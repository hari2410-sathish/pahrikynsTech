import React, { useEffect, useState } from "react";
import {
  Box,
  Typography,
  Paper,
  Table,
  TableHead,
  TableRow,
  TableCell,
  TableBody,
  CircularProgress,
  IconButton,
  Chip,
  Snackbar,
  Alert,
  TextField,
  Select,
  MenuItem,
  Pagination,
  Stack,
  Tooltip,
  InputAdornment,
  Avatar
} from "@mui/material";
import { motion, AnimatePresence } from "framer-motion";

import SearchIcon from "@mui/icons-material/Search";
import VisibilityTwoToneIcon from "@mui/icons-material/VisibilityTwoTone";
import EditTwoToneIcon from "@mui/icons-material/EditTwoTone";
import CurrencyRupeeIcon from "@mui/icons-material/CurrencyRupee";
import ToggleOnTwoToneIcon from "@mui/icons-material/ToggleOnTwoTone";
import ToggleOffTwoToneIcon from "@mui/icons-material/ToggleOffTwoTone";
import AutoStoriesTwoToneIcon from "@mui/icons-material/AutoStoriesTwoTone";
import SchoolTwoToneIcon from "@mui/icons-material/SchoolTwoTone";

import { useNavigate } from "react-router-dom";
import {
  fetchAdminCourses,
  toggleAdminCourseStatus,
} from "../../Adminapi/coursesAdmin";

export default function AllCourses() {
  const navigate = useNavigate();

  const [courses, setCourses] = useState([]);
  const [q, setQ] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
  const [total, setTotal] = useState(0);

  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState({
    open: false,
    msg: "",
    type: "error",
  });

  const pageSize = 10;

  const showToast = (msg, type = "error") =>
    setToast({ open: true, msg, type });

  async function loadCourses(params = {}) {
    try {
      setLoading(true);

      const data = await fetchAdminCourses({
        search: params.search ?? q,
        status: params.status ?? status,
        page: params.page ?? page,
        limit: pageSize,
      });

      setCourses(data.courses || []);
      setTotal(data.total || 0);
    } catch (err) {
      console.error(err);
      showToast("Neural link failed: Curriculum unreachable.");
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => {
    loadCourses();
  }, [page, status]);

  const handleSearch = (e) => {
    e.preventDefault();
    setPage(1);
    loadCourses({ search: q, page: 1 });
  };

  const handleToggleStatus = async (courseId) => {
    try {
      await toggleAdminCourseStatus(courseId);
      showToast("Access protocols updated.", "success");
      loadCourses();
    } catch (err) {
      console.error(err);
      showToast("Override failed.");
    }
  };

  const totalPages = Math.ceil(total / pageSize);

  return (
    <Box sx={{ p: 4 }}>
      {/* ================= HEADER ================= */}
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 6, alignItems: "center" }}>
        <Box>
           <Stack direction="row" spacing={1.5} alignItems="center" mb={1}>
             <AutoStoriesTwoToneIcon sx={{ color: "#00eaff", fontSize: 28 }} />
             <Typography variant="h4" fontWeight={900} sx={{ letterSpacing: "-0.02em", color: "white" }}>Academic Catalog</Typography>
           </Stack>
           <Typography variant="body2" sx={{ opacity: 0.5, fontWeight: 500, color: "white" }}>
             Global curriculum database and enrollment status control.
           </Typography>
        </Box>
        
        <Stack direction="row" spacing={2} alignItems="center">
           <Select
            size="small"
            value={status}
            onChange={(e) => {
              setStatus(e.target.value);
              setPage(1);
            }}
            sx={selectStyle}
          >
            <MenuItem value="all">ALL MODULES</MenuItem>
            <MenuItem value="active">AUTHORIZED</MenuItem>
            <MenuItem value="disabled">REVOKED</MenuItem>
            <MenuItem value="draft">STAGING</MenuItem>
          </Select>

           <TextField
            placeholder="SCAN BY TITLE / KEYWORD..."
            size="small"
            value={q}
            onChange={(e) => setQ(e.target.value)}
            onKeyPress={(e) => e.key === 'Enter' && handleSearch(e)}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <SearchIcon sx={{ color: "rgba(255,255,255,0.3)" }} />
                </InputAdornment>
              ),
            }}
            sx={searchStyle}
          />
        </Stack>
      </Box>

      {/* ================= DATA TABLE ================= */}
      <Paper sx={{ 
        borderRadius: 6, 
        overflow: "hidden", 
        background: "rgba(255,255,255,0.02)", 
        border: "1px solid rgba(255,255,255,0.06)",
        backgroundImage: "none"
      }}>
        <Table>
          <TableHead sx={{ bgcolor: "rgba(255,255,255,0.03)" }}>
            <TableRow>
              <TableCell sx={headerStyle}>CURRICULUM ENTITY</TableCell>
              <TableCell sx={headerStyle}>SECTOR</TableCell>
              <TableCell sx={headerStyle}>FISCAL VALUE</TableCell>
              <TableCell sx={headerStyle}>ENROLLMENT</TableCell>
              <TableCell sx={headerStyle}>STATUS</TableCell>
              <TableCell sx={headerStyle} align="right">RECORD CONTROLS</TableCell>
            </TableRow>
          </TableHead>

          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={6} align="center" sx={{ py: 12 }}>
                  <CircularProgress size={30} sx={{ color: "#00eaff" }} />
                </TableCell>
              </TableRow>
            ) : courses.length === 0 ? (
               <TableRow>
                <TableCell colSpan={6} align="center" sx={{ py: 12, opacity: 0.5, color: "white" }}>
                  <Typography variant="body2" sx={{ fontWeight: 800 }}>MODULE ARCHIVE EMPTY</Typography>
                </TableCell>
               </TableRow>
            ) : (
              <AnimatePresence mode="popLayout">
                {courses.map((c, idx) => {
                  const id = c.id || c._id;
                  const isActive = c.isActive;

                  return (
                    <TableRow
                      component={motion.tr}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, scale: 0.95 }}
                      transition={{ delay: idx * 0.02 }}
                      key={id}
                      hover
                      sx={{ "&:hover": { bgcolor: "rgba(255,255,255,0.03) !important" } }}
                    >
                      <TableCell sx={cellStyle}>
                         <Stack direction="row" spacing={2} alignItems="center">
                            <Avatar sx={{ bgcolor: "rgba(0, 234, 255, 0.05)", color: "#00eaff", border: "1px solid #00eaff33" }}>
                              <SchoolTwoToneIcon fontSize="small" />
                            </Avatar>
                            <Box>
                              <Typography variant="body2" sx={{ fontWeight: 800, color: "white" }}>{c.title}</Typography>
                              <Typography variant="caption" sx={{ opacity: 0.4, color: "white" }}>{c.slug}</Typography>
                            </Box>
                         </Stack>
                      </TableCell>
                      
                      <TableCell sx={cellStyle}>
                         <Chip
                            label={(c.category || "General").toUpperCase()}
                            size="small"
                            sx={{ 
                              bgcolor: "rgba(168, 85, 247, 0.1)", 
                              color: "#a855f7",
                              fontWeight: 900,
                              fontSize: 9,
                              letterSpacing: 0.5,
                              border: "1px solid rgba(168, 85, 247, 0.2)"
                            }}
                          />
                      </TableCell>

                      <TableCell sx={cellStyle}>
                        <Typography variant="body2" sx={{ fontWeight: 900, color: "white" }}>
                          ₹{Number(c.price || c.finalPrice || 0).toLocaleString("en-IN")}
                        </Typography>
                      </TableCell>

                      <TableCell sx={cellStyle}>
                         <Stack direction="row" spacing={1} alignItems="center">
                            <Box sx={{ width: 6, height: 6, borderRadius: "50%", bgcolor: "#00eaff" }} />
                            <Typography variant="body2" sx={{ fontWeight: 700, color: "rgba(255,255,255,0.6)" }}>
                              {c.studentsCount || 0} Learners
                            </Typography>
                         </Stack>
                      </TableCell>

                      <TableCell sx={cellStyle}>
                        <Chip
                          label={isActive ? "AUTHORIZED" : "REVOKED"}
                          size="small"
                          sx={{ 
                            bgcolor: isActive ? "rgba(34, 197, 94, 0.1)" : "rgba(239, 68, 68, 0.1)", 
                            color: isActive ? "#22c55e" : "#ef4444",
                            fontWeight: 900,
                            fontSize: 9,
                            border: `1px solid ${isActive ? "#22c55e44" : "#ef444444"}`
                          }}
                        />
                      </TableCell>

                      <TableCell sx={cellStyle} align="right">
                        <Stack direction="row" spacing={1} justifyContent="flex-end">
                           <Tooltip title="Preview Module">
                              <IconButton
                                size="small"
                                onClick={() => navigate(`/admin/courses/${id}`)}
                                sx={{ color: "#00eaff", bgcolor: "rgba(0, 234, 255, 0.05)" }}
                              >
                                <VisibilityTwoToneIcon fontSize="small" />
                              </IconButton>
                           </Tooltip>

                           <Tooltip title="Edit Manuscript">
                              <IconButton
                                size="small"
                                onClick={() => navigate(`/admin/courses/${id}/edit`)}
                                sx={{ color: "#a855f7", bgcolor: "rgba(168, 85, 247, 0.05)" }}
                              >
                                <EditTwoToneIcon fontSize="small" />
                              </IconButton>
                           </Tooltip>

                           <Tooltip title="Fiscal Adjustment">
                              <IconButton
                                size="small"
                                onClick={() => navigate(`/admin/courses/${id}/price`)}
                                sx={{ color: "#f59e0b", bgcolor: "rgba(245, 158, 11, 0.05)" }}
                              >
                                <CurrencyRupeeIcon fontSize="small" />
                              </IconButton>
                           </Tooltip>

                           <Tooltip title={isActive ? "Deactivate" : "Activate"}>
                              <IconButton
                                size="small"
                                onClick={() => handleToggleStatus(id)}
                                sx={{ color: isActive ? "#22c55e" : "#ef4444" }}
                              >
                                {isActive ? <ToggleOnTwoToneIcon fontSize="small" /> : <ToggleOffTwoToneIcon fontSize="small" />}
                              </IconButton>
                           </Tooltip>
                        </Stack>
                      </TableCell>
                    </TableRow>
                  );
                })}
              </AnimatePresence>
            )}
          </TableBody>
        </Table>
      </Paper>

      {/* ================= PAGINATION ================= */}
      {totalPages > 1 && (
        <Stack alignItems="center" sx={{ mt: 6 }}>
          <Pagination
            count={totalPages}
            page={page}
            onChange={(e, val) => setPage(val)}
            sx={paginationStyle}
          />
        </Stack>
      )}

      {/* ================= TOAST ================= */}
      <Snackbar
        open={toast.open}
        autoHideDuration={3000}
        onClose={() => setToast({ ...toast, open: false })}
      >
        <Alert severity={toast.type} sx={{ borderRadius: 3, fontWeight: 800 }}>{toast.msg}</Alert>
      </Snackbar>
    </Box>
  );
}

const headerStyle = {
  color: "rgba(255,255,255,0.4)",
  fontWeight: 900,
  fontSize: 10,
  letterSpacing: 1.5,
  borderBottom: "1px solid rgba(255,255,255,0.05)",
  py: 3
};

const cellStyle = {
  borderBottom: "1px solid rgba(255,255,255,0.03)",
  py: 2.5
};

const selectStyle = {
  minWidth: 160,
  color: "white",
  fontWeight: 900,
  fontSize: 12,
  borderRadius: 3,
  bgcolor: "rgba(255,255,255,0.03)",
  border: "1px solid rgba(255,255,255,0.1)",
  "& .MuiOutlinedInput-notchedOutline": { border: "none" },
  "& .MuiSvgIcon-root": { color: "#00eaff" }
};

const searchStyle = {
  "& .MuiOutlinedInput-root": {
    borderRadius: 3,
    color: "white",
    width: 300,
    bgcolor: "rgba(255,255,255,0.03)",
    "& fieldset": { borderColor: "rgba(255,255,255,0.1)" },
    "&.Mui-focused fieldset": { borderColor: "#00eaff" },
    "&:hover fieldset": { borderColor: "rgba(255,255,255,0.2)" }
  },
  "& .MuiInputBase-input::placeholder": { color: "rgba(255,255,255,0.3)", opacity: 1, fontWeight: 700, fontSize: 10 }
};

const paginationStyle = {
  "& .MuiPaginationItem-root": { color: "white", fontSize: 12, fontWeight: 800 },
  "& .Mui-selected": { bgcolor: "rgba(0, 234, 255, 0.1) !important", color: "#00eaff", border: "1px solid #00eaff44" }
};
